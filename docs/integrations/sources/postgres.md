...
Alternatively, keep `REPLICA IDENTITY DEFAULT` and set the **Columns to re-select for unchanged TOAST values** option (`reselect_columns`) on the CDC replication method to a comma-separated list of columns in the form `schema.table:column`, for example `public.check_ins:data,public.benefit_checks:result_data`. When Postgres omits an unchanged TOAST-ed value from an UPDATE event, the connector re-reads just those columns from the source by primary key (using Debezium's [Reselect Columns post processor](https://debezium.io/documentation/reference/stable/post-processors/reselect-columns.html)) instead of emitting the placeholder `__debezium_unavailable_value`. This trades one indexed lookup per affected UPDATE event for the extra WAL that `REPLICA IDENTITY FULL` would generate, so it suits tables that are updated far more often than their large columns change. Only the listed columns are ever re-selected, columns that are legitimately `NULL` are never re-selected, and a row that disappears between the event and the lookup is logged and left as the placeholder. The connector's database user needs `SELECT` on the listed tables. Without the option, every column type consistently receives the `__debezium_unavailable_value` placeholder for an unchanged TOAST-ed value (prior to 3.9.0, `text` and `varchar` columns received a `java.lang.Object@...` string instead, and unchanged TOAST-ed `bytea` or array columns failed the sync).
...
|---------|------------|----------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 3.9.0   | 2026-09-15 | [TBD](https://github.com/airbytehq/airbyte/pull/TBD)     | Add `reselect_columns` CDC option to re-read unchanged TOAST-ed values from the source instead of emitting `__debezium_unavailable_value`; fix unchanged TOAST-ed `text`/`varchar`/`bytea` columns being emitted as `java.lang.Object@...` instead of the placeholder, and unchanged TOAST-ed arrays failing the sync. |
